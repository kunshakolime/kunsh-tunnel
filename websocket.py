...
class Config:
    ...
    _default_config_yaml = """# WebSocket Tunnel Server Configuration
ports: # Port Configuration
  http: [80, 8080, 8000, 8880]      # HTTP ports (plain text)
  https: [443, 8443]           # HTTPS ports (SSL/TLS)
...
"""
    ...
    def _load_config(self) -> Dict[str, Any]:
        """Load config from YAML file, create if missing"""
        if not Path(self.config_file).exists():
            logging.warning(f"{self.config_file} not found, creating default config")
            try:
                with open(self.config_file, 'w', encoding='utf-8') as f:
                    f.write(self._default_config_yaml)
                logging.info(f"✓ Created {self.config_file}")
            except Exception as e:
                logging.error(f"Failed to create {self.config_file}: {e}")
